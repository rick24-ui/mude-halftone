...
// ---------- estado compartilhado (Pontilhismo + Texto) ----------
...
// ---------- seções (presets + ajustes) ----------

export function ControlsSections({ ctl }: { ctl: ControlsState }) {
  const {
    p,
    set,
    presetId,
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              <button
                ...
                className="rounded-lg bg-[var(--panel-2)] py-2.5 text-xs font-medium hover:bg-[var(--line)] disabled:opacity-40"
              >
      ...
      <Section title="Biblioteca de Estilos">
        <div className="flex gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-lg bg-[var(--panel-2)] px-2 py-1.5 text-[12px] outline-none placeholder:text-muted focus:ring-1 focus:ring-red"
          />
          <button
            onClick={() => {
              saveStyle(styleName);
            ...
            className="rounded-lg bg-red px-3 py-1.5 text-[11px] font-medium text-white hover:opacity-90"
          >
            Salvar
          </button>
        </div>
        {library.length === 0 ? (
          ...
          <div className="space-y-1">
            {library.map((s) => (
              <div
                key={s.id}
                className="group flex items-center justify-between rounded-lg border border-line px-2 py-1.5"
              >
                <button
...
export function ControlsExportFooter({ ctl }: { ctl: ControlsState }) {
  const { source, copied, scale, setScale, runExport } = ctl;
  ...
  return (
    <div className="border-t border-white/10 px-4 py-4">
        ...
        <div className="flex gap-1">
          {[1, 2, 4].map((s) => (
            <button
              key={s}
              onClick={() => setScale(s)}
              className={`rounded-lg px-2 py-0.5 text-[11px] ${scale === s ? "bg-red text-white" : "bg-[var(--panel-2)] text-muted"}`}
            >
              {s}×
            </button>
          ))}
      ...
      <button
        ...
        disabled={!source}
        className="mt-2 w-full rounded-lg bg-red py-2.5 text-xs font-medium text-white hover:opacity-90 disabled:opacity-40"
      >
...
// ---------- painel (uso autônomo — aba Pontilhismo) ----------
